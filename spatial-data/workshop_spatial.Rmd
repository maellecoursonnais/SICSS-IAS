...
[OpenStreetMap (OSM) is a free, open geographic database updated and maintained by a community of volunteers via open collaboration.](https://en.wikipedia.org/wiki/OpenStreetMap).
...
```{r}
library(pacman)
p_load(osmdata, tidyverse, sf, tmap, leaflet)
```
...
nkpg_schools$osm_multipolygons

nc <- st_read(shp, quiet = TRUE)
leaflet() %>%
  addPolygons(data = nc, stroke=T)

qtm(nkpg_schools$osm_points[!is.na(nkpg_schools$osm_points$name), ])
```
