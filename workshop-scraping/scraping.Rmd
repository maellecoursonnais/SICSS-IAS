...
#1.
html_example %>% 
  #Text is situated in the title and the body
...
- Response: APIs respond to an request with a HTTP response. The response usually contains a status message, meta data, as well as the requested piece of information or an error message. Most often, API responses come in JSON or XML format. 
...
# retrive number of articles, number of pages, and construct dataframe
...
hern_articles <- hern_search %>% 
  select(section_id, web_publication_date, web_title) %>% # select columns of interest
  filter(web_publication_date >= as.Date("2014-01-01"),
         web_publication_date <= as.Date("2022-12-31")) %>% # filter time window of interest
  mutate(date = as.Date(web_publication_date, format = "%Y-%m-%d %h:%m:%s CEST")) %>% 
  mutate(year = lubridate::year(date), month = lubridate::month(date), day = lubridate::day(date)) # get year, month, and day
hern_articles$date <- as.Date(sprintf("%04d-%02d-15", hern_articles$year, hern_articles$month)) # construct date variable
